...
resource "aws_appautoscaling_scheduled_action" "app_autoscale_time_up" {
  ...
  scalable_target_action {
    min_capacity = "${aws_appautoscaling_target.app_scale_target.min_capacity+2}"
    max_capacity = "${aws_appautoscaling_target.app_scale_target.max_capacity+2}"
  }
}
