...
resource "aws_cloudwatch_event_rule" "ecs_event_stream" {
  name        = "wordpress-ecs-event-stream"
  description = "Passes ecs event logs for wordpress to a lambda that writes them to cw logs"
...
data "archive_file" "lambda_zip" {
  ...
  output_path             = "lambda-wordpress.zip"
}
...
resource "aws_lambda_function" "ecs_event_stream" {
  function_name    = "wordpress-ecs-event-stream"
...
resource "aws_cloudwatch_dashboard" "ecs-event-stream" {
  dashboard_name = "wordpress-ecs-event-stream"
  ...
  "widgets": [
    {
      ...
      "height": 18,
      "properties": {
        "query": "SOURCE '/aws/lambda/wordpress-ecs-event-stream' | fields @timestamp as time, detail.desiredStatus as desired, detail.lastStatus as latest, detail.stoppedReason as reason, detail.containers.0.reason as container_reason, detail.taskDefinitionArn as task_definition\n| filter @type != \"START\" and @type != \"END\" and @type != \"REPORT\"\n| sort detail.updatedAt desc, detail.version desc\n| limit 100",
        "region": "us-west-2",
        "title": "ECS Event Log"
      }
    }
